...
Актуально на текущий коммит (0.7.0-dev)
...
**Решение:**
- Версия берётся из CMake (`project(... VERSION 0.7.0)`) через
...
`std::ofstream`) файл оставался битым. При следующем старте
`parse_state()` мог упасть или вернуть мусор.

**Решение:**
- `saveState()` пишет в `state.json.tmp`, затем атомарно переименовывает
  через `fs::rename()` (на той же ФС rename — атомарная операция).
- `loadState()` обёрнут в try-catch: при любой ошибке парсинга битый
  файл удаляется и возвращаются значения по умолчанию.
